...
public class TestQueue {
	public static void main(String[] args) {
		...
		Layer layer1 = LayerUtil.build(3, 3, 1);
		Layer layer2 = LayerUtil.build(3, 3, 2);
		Layer layer3 = LayerUtil.build(3, 3, 3);
		
		map.getList().add(layer1);
		map.getList().add(layer2);
		map.getList().add(layer3);
		List<Layer>list = map.getList();
		ExtraLayer extraLayer = ExtraLayerUtil.build(map.getExtraFloorHeight());
